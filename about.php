<?php ?>
<!DOCTYPE html>
<html lang='ja'>
       <head>
            <meta charset='UTF-8'>
            <meta name='viewport' content='width=device-width'>
            <meta name='keywords' content='数学、医学部、大学受験、難関大学、プログラミング教育'>
            <title></title>
            <link rel='stylesheet' href='styles.css'>
       </head>
       <body>
         <header>
               <h1>学習舎の<span>オンライン数学講座</span></h1>
         </header>
         <div id='toolbar'>
                <section class='toolbar_name'><a href='lessonstyle.php'>講座の受講style</a></section>
                <section class='toolbar_name'><a href='syllabus.php'>講座の内容</a></section>
                <section class='toolbar_name'><a href='trial.php'>体験授業</a></section>
         </div>


            <section id='container'>
                <article class='box'>
                      <h3>学習舎は「オンラインのみの数学専門塾」です</h3>
                       <p>日々の定期試験対策から医学部の受験対策まで対応する数学のオンライン塾です。<br>
                          「分かる」だけでなく「できる」ことを目指す指導を特徴としています。<br>
                           その要点は次の２点に集約されています。
                              <ul>
                                     <li>1.リアルタイムの個別指導を行う</li>
                                     <li>2.学んだ内容を自分のものにするために課題添削を行う</li>
                                     <li>3.質問があればすぐに解決させる</li>
                              </ul>
                       </p>
                </article>
                <article class='box'>
                       <dl>
                              <dt>代表紹介</dt>
                              <dd>名前:永田　卓</dd>
                              <dd>略歴：1969年東京都生まれ。<br>
                                  「市進予備校」など大手予備校を皮切りに大学受験の数学を教える教壇に立つこと30年ほど。<br>
                                   この間数学ができるようにすることを目指して指導を続け、オンラインでもこれが実現<br>
                                   可能であることからオンラインによる数学塾を開催。<br>
                                   現在は教える教科と学年を拡大して指導中。中学の理科と数学、高校の物理、などの試験対策<br>
                                   と補習を手掛けている。<br>
                                     趣味は料理と海をめぐる旅行。
                              </dd>
                       </dl>
                </article>
                   <h3 class='goto_index'><a href='index.php'>トップページに戻る</a></h3>
            </section>
            <footer>
                <h3>学習舎の数学講座 &lt; &copy; &gt;</h3>
            </footer>
       </body>
</html>
